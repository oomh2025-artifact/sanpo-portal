...
【指示】
1. まず、厚生労働省(mhlw.go.jp)や安全衛生情報センター(jaish.gr.jp)などの公的機関サイトをWeb検索して、関連するガイドライン、通達、法令、統計データを詳しく調べてください。

2. 回答は以下の点を押さえて、**詳細かつ具体的に**作成してください（600-800字程度）：
   - 現状の数値データや統計（具体的な数字を含める）
   - 法的根拠や制度の詳細
   - 実務での具体的な対応手順
   - 最新の動向や改正情報

3. 論文情報がある場合は、研究で明らかになった知見も含めてください。

4. 産業医や産業保健スタッフが**そのまま実務で使える**レベルの具体性を持たせてください。

【出力形式の注意】
- 「###」「##」などのMarkdown見出し記法は使わないでください
- 見出しは「■現状と課題」「■対策のポイント」のように■を使ってください
- 太字は**このように**使ってOKです
- 箇条書きは「・」を使ってください
- プレーンテキストで読みやすく整形してください
- 参考URLは最後にまとめて記載してください`
        }]
      });
